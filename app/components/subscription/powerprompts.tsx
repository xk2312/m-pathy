...
export default function PowerPrompts() {
  ...
  const onAsk = (text: string) => {
    ...
    router.push(`/subscription/page2?prefill=${q}`); // optional &auto=1 später möglich
  };

  // 🔧 FIX: Titel & Hint vor dem JSX berechnen (sonst TS/JSX-Fehler)
  const title = t("pp.title");
  const hint = t("pp.hint");
    ...
    <section aria-label="Power Prompts" className="relative">
      ...
      <div className="text-center mb-6">
        <h2 className="text-[clamp(22px,4vw,44px)] leading-tight font-semibold text-white">
          {title && title !== "pp.title" ? title : "Power Prompts"}
        </h2>
        <p className="mt-2 text-white/70">
          {hint && hint !== "pp.hint" ? hint : "Pick a category and start with a magical question."}
        </p>
      </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {CATS.map(({ id, label }) => {
            ...
            return (
              ...
              >
                <span className={activeNow ? "text-cyan-300" : "text-white/80"}>{ICONS[id]}</span>
