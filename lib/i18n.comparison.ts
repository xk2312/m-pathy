/*** =======================================================================
 *  INVENTUS INDEX - lib/i18n.comparison.ts
 *  i18n Vergleichsdict · EU AI Act · 13 Sprachen
 * ======================================================================= 
 *
 *  [ANCHOR:0]  DATEIKONTEXT
 *              - Exportiert ein einziges Objekt: export const dict = { ... }
 *              - Enthält die i18n Texte für das EU AI Act Comparison Modul.
 *              - Struktur pro Sprache:
 *                { kicker, title, intro, table.columns, groups, legend,
 *                  footer, maios_note }
 *              - Reines Datenmodul ohne Funktionen oder Imports.
 *
 *  [ANCHOR:1]  ENGLISCHER BASISBLOCK (en)
 *              - kicker: "EU AI ACT ALIGNMENT"
 *              - title: "Regulatory capability comparison"
 *              - intro: Erläuterung, dass der Vergleich evidenzbasiert
 *                und zeitgebunden ist.
 *              - table.columns:
 *                  criterion, maios, mpathy, chatgpt, copilot, gemini
 *              - groups:
 *                  governance, transparency, oversight, risk, data, post_market
 *                jeweils mit:
 *                  title: Gruppenname
 *                  rows: Record<string, string> mit 5 Zeilen pro Gruppe
 *                    z. B. governance.rows:
 *                      purpose, accountability, role_separation,
 *                      architecture, change_management
 *              - legend:
 *                  supported, not_supported
 *              - footer:
 *                  sources_title, sources_note, verification_date_label
 *              - maios_note:
 *                  title, body
 *              → en ist die voll ausgearbeitete Referenzsprache
 *                mit sämtlichen Zeilenbeschreibungen.
 *
 *  [ANCHOR:2]  DEUTSCHER BLOCK (de)
 *              - kicker, title, intro übersetzt.
 *              - table.columns: alle Spalten korrekt abgebildet.
 *              - groups:
 *                  governance, transparency, oversight, risk, data, post_market
 *                jeweils nur:
 *                  title: übersetzter Gruppenname
 *                KEINE rows definiert.
 *              - legend, footer, maios_note: vollständig übersetzt.
 *              → de nutzt aktuell nur Gruppentitel ohne Zeilenbeschreibungen.
 *
 *  [ANCHOR:3]  WEITERE SPRACHEN MIT TITELN OHNE ROWS
 *              - fr, es, it, pt, nl, ru, zh, ja, ko, ar, hi
 *              - Durchgehend gleiche Struktur:
 *                  kicker, title, intro
 *                  table.columns komplett
 *                  groups:
 *                    governance, transparency, oversight, risk, data, post_market
 *                    jeweils NUR title, KEINE rows
 *                  legend mit supported / not_supported
 *                  footer mit sources_title, sources_note, verification_date_label
 *                  maios_note mit title und body
 *              → Alle nicht englischen Sprachen haben bisher
 *                keine fein granulierten Requirement Texte pro Zeile.
 *
 *  [ANCHOR:4]  SPRACHABDECKUNG UND SCHLÜSSEL
 *              - dict enthält exakt 13 Sprachkeys:
 *                en, de, fr, es, it, pt, nl, ru, zh, ja, ko, ar, hi
 *              - Spaltenkeys unter table.columns sind in allen Sprachen identisch:
 *                criterion, maios, mpathy, chatgpt, copilot, gemini
 *              - Gruppenschlüssel sind in allen Sprachen identisch:
 *                governance, transparency, oversight, risk, data, post_market
 *              → Technische Key Ebene ist konsistent,
 *                Inhaltstiefe (rows) ist nur in en ausgefüllt.
 *
 *  [ANCHOR:5]  ERWARTETE KOPPLUNG ZUR COMPARISON KOMPONENTE
 *              - Comparison Komponente erwartet:
 *                  locale.table.columns.*
 *                  locale.groups[groupKey].title
 *                  locale.groups[groupKey].rows[rowKey]
 *                  locale.footer.*
 *              - groupKey und rowKey müssen mit:
 *                  - comparisonMatrix[groupKey][rowKey]
 *                  - UI Rendering der Zeilen
 *                übereinstimmen.
 *              → en erfüllt diese Voraussetzung bereits vollständig,
 *                alle anderen Sprachen liefern nur titles auf Gruppenebene.
 *
 * =======================================================================
 *  ERKENNBARER FEHLER- UND DRIFTRISIKO-BEREICH (Inventur, ohne Lösung)
 *
 *  1) Inhaltliche Asymmetrie en vs. alle anderen Sprachen
 *     - Nur en definiert rows Texte für die Anforderungen je Gruppe.
 *     - de und die weiteren 11 Sprachen haben aktuell keine rows.
 *     - Risiko:
 *       - Wenn das UI locale.groups[groupKey].rows[rowKey] für andere
 *         Sprachen erwartet, können:
 *         a) Zeilen fehlen oder leer sein.
 *         b) Fallbacks oder defensive Guards unerwartete Zustände erzeugen.
 *
 *  2) Mehrsprachige Konsistenz der Requirements
 *     - Die semantischen Anforderungen (purpose, accountability, logging etc.)
 *       sind in en sauber formuliert, aber noch nicht in den anderen Sprachen.
 *     - Gefahr:
 *       - User in nicht englischen Locales sehen nur Gruppentitel ohne
 *         konkrete Requirement Beschreibung pro Zeile oder erhalten
 *         generische Fallback Texte.
 *
 *  3) Pflege und Erweiterung
 *     - Jede Erweiterung der Comparison Matrix (neue groupKey oder rowKey)
 *       erfordert:
 *         a) Ergänzung der rows in en,
 *         b) anschließende Übersetzung in alle 12 weiteren Sprachen.
 *     - Ohne klaren Prozess besteht Driftgefahr zwischen Matrix und i18n,
 *       insbesondere wenn Zeilen in en hinzugefügt oder geändert werden,
 *       aber nicht synchron in die anderen Blöcke übertragen werden.
 *
 * ======================================================================= */
export const dict = {
  en: {
